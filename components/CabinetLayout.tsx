...
const menu = [
  { key: 'profile', label: 'Мої дані', path: '/cabinet-doctor/profile' },
  { key: 'consults', label: 'Мої консультації', path: '/cabinet-doctor/consults' },
  { key: 'finance', label: 'Мої фінанси', path: '/cabinet-doctor/finance' },
  { key: 'schedule', label: 'Мій робочий графік', path: '/cabinet-doctor/schedule' },
]
...
export default function CabinetLayout({
  ...
  const [nearestConsults, setNearestConsults] = useState<any[]>([])
  ...
  useEffect(() => {
    if (!user) return
    // Завантаження найближчих консультацій
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '2rem auto', minHeight: 700, fontFamily: 'Arial, sans-serif', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 52 }}>
        <div style={{ fontSize: 30, fontWeight: 700, color: '#232b3e' }}>Кабінет лікаря</div>
        <button
      ...
      {/* Топ-блок - Найближчі консультації */}
      <div style={{
        background: '#f2f8fd', borderRadius: 15, margin: '30px 0 36px 0', padding: '22px 28px',
        border: '1px solid #e8eef7', minHeight: 60
      }}>
        <div style={{ fontWeight: 700, fontSize: 21, marginBottom: 3 }}>Найближчі консультації</div>
        {nearestConsults.length === 0
          ? <div style={{ color: '#7c8797', fontSize: 17, paddingTop: 5 }}>Немає запланованих консультацій</div>
          : nearestConsults.map((c, i) => (
            <div key={c.id} style={{ fontSize: 16, marginTop: i === 0 ? 4 : 10 }}>
              {new Date(c.date).toLocaleDateString()} {new Date(c.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} &mdash; {c.patient?.first_name} {c.patient?.last_name}
            </div>
          ))}
      </div>
      {/* Layout: меню + контент */}
