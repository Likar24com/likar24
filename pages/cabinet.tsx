...
export default function PatientCabinet() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      // 2) Fetch appointments with generic
      ...
      const arr: Appointment[] = apps || []
      setAppointments(arr)

      // 3) Compute next appointment
      const next = arr
  ...
  const confirmReschedule = async () => {
    ...
    setAppointments(xs => xs.map(a =>
      a.id === rescheduleApp.id ? { ...a, date_time: newDateTime } : a
    ))
