...
export default function PatientCabinet() {
  ...
  const sidebarButton = (active:boolean) => ({
    ...
    border:'none', borderRadius:4, marginBottom:8,
    cursor:'pointer'
  })
  ...
  useEffect(() => {
    ;(async () => {
      ...
      const { data: pr, error: prErr } = await supabase
        ...
        .eq('id', user.id)
        .single()
      if (prErr) console.error('fetch profile error', prErr)
      if (pr) {
        setProfile(pr)
      ...
      if (appsErr) console.error('fetch apps error', appsErr)
      const arr = apps || []
      setAppointments(arr)

      // — 3) обчислюємо найближчу —
      const next = arr
  ...
  const cancelAppointment = async(id:string) => {
    await supabase
      ...
      .eq('id', id)
    setAppointments(xs =>
      xs.map(a => a.id===id ? { ...a, status:'скасовано' } : a)
    )
        ...
        <div style={{
          ...
          <div style={{ background:'#fff', padding:20, borderRadius:6, minWidth:300 }}>
            ...
            <div style={{ textAlign:'right' }}>
              ...
              <button onClick={()=>setRescheduleApp(null)}>
                Відмінити
              </button>
            </div>
          </div>
