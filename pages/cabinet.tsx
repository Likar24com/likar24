  ...
  const saveProfileData = async(e:React.FormEvent)=>{
    ...
    try {
      await supabase
        .from('users')
        .update({
          first_name, last_name:lastName||null, middle_name,
          birth_date, weight:+weight, country,
          allergies, chronic_diseases:chronic||null, email
        })
        .eq('id', userId!)
        ...
        <article style={{flex:1, minHeight:600, transition:'opacity .2s'}}>
          ...
          <div style={{display: activeMenu==='my-records'?'block':'none'}}>
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              ...
              <tbody>
                {appointments
                  .filter(a=> recordsTab==='upcoming'? a.status==='очікується' : a.status==='завершено')
                  .map(a=>(
                    <tr key={a.id}>
                      <td style={thTdStyle}>
                ...
                {appointments.filter(a=> recordsTab==='upcoming'? a.status==='очікується':a.status==='завершено').length===0 && (
                  <tr>
                    <td colSpan={5} style={{textAlign:'center',padding:20}}>Немає записів.</td>
                  </tr>
                )}
              </tbody>
            </table>
